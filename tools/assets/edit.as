...
_global.gDesc = "";
_global.gRules = "";
_global.gPlayerstart = new Array();
...
var ExportCallback = function()
{
	...
	this.ho += "&sextent=3&\n";
	this.ho += "&spawners="+gSpawners+"&\n";
	this.ho += "&pextent=3&\n";
	this.ho += "&playersstart="+gPlayerstart+"&\n";
...
var MouseDownCallback = function()
{
	...
	{
		if( gCurrentProp._name == "spawning" )
		{
			var depth = gRef.gUserMap.getNextHighestDepth() + gNumEntities
			gSpawners.push( gCurrentProp._rotation );
			gSpawners.push( gCurrentProp._x );
			gSpawners.push( gCurrentProp._y );
			gCurrentProp._name = "ent"+gNumEntities;
			gCurrentProp = null;
			gNumEntities++;
			return;
		}
		if( gCurrentProp._name == "playerspawning" )
		{
			gPlayerstart[0] = ( gCurrentProp._rotation );
			gPlayerstart[1] = ( gCurrentProp._x );
			gPlayerstart[2] = ( gCurrentProp._y );
			gCurrentProp = null;
			return;
		}
		var i, j;
		...
		if( gCurrentProp._name == "de" )
		{
			...
			gEntities.push( gCurrentProp._x );
			gEntities.push( gCurrentProp._y );
			gEntities.push( gCurrentProp._xscale );
		}
		//Release
		gCurrentProp = null;
		return;
	}
	if( gCurrentProp == null && !Key.isDown( Key.SPACE ) )
	{
		var i;
...
kl.onKeyDown = function()
{
	...
	if( !gKey_spawners && Key.isDown( 88 ) )
	{
		if( gCurrentProp._name != "spawning" )
		{	
			gCurrentProp.removeMovieClip();
			gRef.UserMap.attachMovie( "spawners", "spawning", gRef.UserMap.getNextHighestDepth() );
			gRef.UserMap["spawning"].gotoAndStop( 3 );
			gCurrentProp = gRef.UserMap["spawning"];
		}
		else
		{	
			gCurrentProp.removeMovieClip();
			gRef.UserMap.attachMovie( "spawners", "playerspawning", gRef.UserMap.getNextHighestDepth() );
			gRef.UserMap["playerspawning"].gotoAndStop( 2 );
			gCurrentProp = gRef.UserMap["playerspawning"];
		}
		gKey_spawners = true;
	}
	...
	mouseListener.onMouseWheel = function(delta:Number) {
    	gCurrentProp._xscale += delta;
		gCurrentProp._yscale += delta;
	};
